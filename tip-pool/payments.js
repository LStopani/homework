...
paymentTbody.addEventListener('click', removePayment);
...
function appendPaymentTable(curPayment) {
  ...
  newDelete.setAttribute('id', `${paymentId}`)
  newDelete.innerHTML = 'Delete'

  appendTd(newTr, '$' + curPayment.billAmt, `billDom${paymentId}`);
  appendTd(newTr, '$' + curPayment.tipAmt, `tipDom${paymentId}`);
  appendTd(newTr, curPayment.tipPercent + '%', `percentDom${paymentId}`);
...
function removePayment(e) {
  ...
  const roundup = document.getElementById(`payment${e.target.id}`);
  roundup.remove();
  delete allPayments[`payment${e.target.id}`];
  console.log(e);
  updateSummary();
  updateServerTable();
};